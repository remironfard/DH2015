...
Our field is based on the rehearsals of two performing shows: \emph{Chatte sur un toit brûlant}, written by Tenessee Williams and directed by Claudia Stavisky, and \emph{Elena}, composed by Francesco Cavalli, directed by Jean-Yves Ruf and conducted by Leonardo García Alarcón.
...
Simultaneously to the video capture, the operator proceeded to the exhaustive description of the rehearsals thanks to the note taking tool. The data produced during this annotation process are synchronized with the video stream and encoded into an XML format based on the cinelab data model\footnote{Cinelab Data Model - \url{http://liris.cnrs.fr/advene/cinelab/}}.
...
\todo{mise en forme tableau}
\begin{tabular}{|l|c|c|c|}
\hline 
  & Chatte sur... & Elena & Total \\ 
\hline 
Number of video & 70 & 100 & 170 \\ 
\hline 
Total duration (in hours) & 242 & 177 & 419 \\ 
\hline 
Total volume (in To) & 2,47 & 1,60 & 4,07 \\ 
\hline 
Number of annotation & 3530 & 6968 & 10498 \\ 
\hline
%\caption{Few facts on the corpus}
%\label{tab:table_facts}
\end{tabular} 

The result of this synchronous annotation was to augment the video stream with semantic metadatas, also considered as intra-video index, making the video corpus easily searchable with a proper search engine.

\subsubsection{Data model} Figure \ref{fig_data_model} describes the main elements of the data model used by the note taking tool. This model results from a workshop analysis, gathering several kinds of attendees (researchers, producers, dramatists, film editors) - each kind having different requirements concerning rehearsals annotation capabilities. It focuses on annotations that have to be created in real-time, i.e. during the rehearsal. \newline
\textbf{Model description}~A session annotation is linked with a video file and represents a rehearsal session (typically lasting a half-day or a day). A session annotation is connected to a place (where the rehearsal takes place) and to a creation (linked to a work and castings). A session annotation is made up of chapters that represent performance or discussion times. A chapter contains contextual data (i.e. with or without costumes, lights, sets and musical inserts) and may reference concerned part of the work (e.g. the act, scene, line numbers corresponding to the beginning/ending of the section of play - \textit{service begin/end}). Chapters also may contain moments of interest (so called categories).
\begin{figure}[ht]
\centering
...
\end{figure}
\subsubsection{Practical usage of the model} Generally speaking, at the beginning of a rehearsals captation process for a given work, an operator first fills into the note taking tool some time-stable data : the creation, work, actors, characters, rehearsals place. Some others data can next be filled or updated during a rehearsal session - e.g. contextual data (costumes, sets, etc.) about a given chapter (inherited from a previous one if any).
%BE on peut pour gagner de la place juste mettre les éléments Session, Chapter, Moment of interest..

\subsubsection{Emerged categories} Each annotation process has revealed a set of categories associated to the interpretation type of annotation, reflecting both the related performing art and the personal point of view of the operator. The tables \todo{to be linked to table figure} present the richness of the annotations and the potential of the archive for further researches in genetic analysis of creative works.
\begin{tabular}{|l|c|}
\hline 
...
After the rehearsals, we centralized the datas on a server for video encoding and annotation metadatas conformation. Due to the tied agenda of the projects, the annotation software did not implement a strict certification on data input. Therefore, as a preliminary step before the ingest of the corpus into the applicative platform, manual cleaning and conforming were executed on the metadatas, such as merging similar categories, uniformising the text reference format, correcting mispelling, etc.
...
Based on an off-the-shelf speaker recognition toolkit, we built a play script following algorithm where rehearsals are assumed to be generated by a probabilistic generative model.  Thus, each  line in the play script is a state 
in a semi-Markov model with a duration probably and a speaker voice probability. In our experiments, this methods outperformed state-of-the-art speaker diarization methods based on HMM, which do not have a duration model.
...
The project built on the applicative plateforme Ligne de temps\footnote{Ligne de temps - \url{http://www.iri.centrepompidou.fr/outils/lignes-de-temps/}} to implement specific modules dedicated to fine-indexed video archive of performance rehearsals.

\subsubsection{Ingest}
The ingest operation was made possible thanks to the preceding data conformation. It consisted in automatic referencing the video files as media contents into the platform and in associating and synchronizing the annotation metadatas to the media contents. Ligne de temps provides an applicative environnement to search, consult, edit and publish the archive, therefore bring the rehearsal annotation data from static to dynamic. \linebreak
However theater and opera archives were separated in two distinct points of access, due to usage scenari and to differences in data format. Indeed, the search engine and further applications require a stable model of data to offer optimized user experience.
...
1) left part : searching chapters with rehearsal context data such as date of rehearsal, character(s) on stage, lines of the script, type of chapters, etc. , providing a sortable list of chapters,
2) right part : searching interpretation annotations with full text form and/or with finished list of categories, providing a sortable list of annotations.

Thanks to the synchronization of metadatas to the video stream, a search result list from either part can be used to send a new request. For instance, from the result list of annotation (right part), it is possible to extract all chapters with corresponding timecode (left part).
