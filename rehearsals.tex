...
\subsubsection{Emerged categories}

Each annotation process has revealed a set of categories associated to the interpretation type of annotation, reflecting both the related performing art and the personal point of view of the operator. The tables \todo{to be linked to table figure} present the richness of the annotations and the potential of the archive for further researches in genetic analysis of creative works.
...
\caption{Annotation categories and number of occurrences \todo{Better caption}}
\end{table} 
...
The ingest operation was made possible thanks to the preceding data conformation. It consisted in automatic referencing the video files as media contents into the platform and in associating and synchronizing the annotation metadata to the media contents. \emph{Ligne de temps} provides an applicative environment to search, consult, edit and publish the archive, therefore bring the rehearsal annotation data from static to dynamic. \linebreak
...
\begin{figure}[htb!]
  ...
  \caption{\todo{Commit the file screenshot, missing from the repo} Multi-faceted search engine for the theater archive}
...
1) left part: searching chapters with rehearsal context data such as date of rehearsal, character(s) on stage, lines of the script, type of chapters, \emph{etc.}, providing a sortable list of chapters,
...
Thanks to the synchronization of metadata to the video stream, a search result list from either part can be used to send a new request. For instance, from the result list of annotation (right part), it is possible to extract all chapters with corresponding timecode (left part).
