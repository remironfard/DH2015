...
\caption{Three successive steps in the rehearsal process: (a) Table readings; (b) Studio rehearsals; (c) Dress rehearsals on stage.}
...
As early as 1966, Jean-Luc Godard offered a remarkably different suggestion for recording theatre when he remarked: {\em ``Why do theater people never film their performances to keep  them as an archive? This would be very simple: put the camera in the middle of the orchestra seats with a medium lens---not a zoom lens, because it would already be making choices and propose an interpretation''}~\cite{Godard66}. Accordingly, our approach in this project has been the record the rehearsals from a single viewpoint (the director's seat) with the highest possible video resolution and to augment them with detailed, high-level  annotations that make it possible to quickly  search, browse and even re-edit the archive interactively. 
...
%\todo{not sure about the interest of those 2 paragraphs in this paper. BE: +1 ; je propose de supprimer ces deux paragraphes} It is a difficult task to create recorded performances to match up to the live experience
...
{
\small
\begin{tabular}{lp{2cm}lp{2cm}lp{2cm}lp{2cm}l}
\hline 
...
Despite the challenging environment, our method provides very good results for the tracking of the six actors, where generic state-of-the-art tracking methods quickly drifted away from the target and were unstable both in presence of occlusion and fast movements \cite{Gandhi13,Migniot15}.
...
\subsection{Fine-grained visualization and browsing capacities}
...
\subsection{Web Components-based approach}
